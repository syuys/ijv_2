...
for sessionID in sessionIDSet:
    ...
    if detectorNA != 0.22:
        ...
        raw, reflectance, reflectanceMean, reflectanceCV, totalPhoton, groupingNum = postprocess.analyzeReflectance(sessionID, mua=muaUsed, detectorNA=detectorNA)
...
popt, pcov = curve_fit(expfit, sdsSet, projectRmeanCV)
projectRmeanCVfit = expfit(sdsSet, *popt)
...
popt, pcov = curve_fit(expfit, sdsSet, CVmean)
CVfit = expfit(sdsSet, *popt)
...
contrastType = "(Rmax-Rmin) / Rmean"
if contrastType == "(Rmax-Rmin) / Rmean":
    contrast = (Rmax-Rmin) / Rmean
elif contrastType == "Rmax / Rmin":
    contrast = Rmax / Rmin
else:
    raise Exception("Contrast type not existed !")
    
cnr = contrast / projectRmeanCVfit
ax2 = ax1.twinx()
ax2.set_ylabel(f"{contrastType}  [-]")
