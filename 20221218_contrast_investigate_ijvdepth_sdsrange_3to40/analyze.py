...
outputPath = "/media/md703/Expansion/syu/ijv_2_output/20221218_contrast_investigate_ijvdepth_sdsrange_3to40"
sessionIDSet = glob(os.path.join(outputPath, "*"))
sessionIDSet = [sessionID.split("/")[-1] for sessionID in sessionIDSet]
sessionIDSet.sort(key=lambda x: (int(x.split("_")[-2]), -ord(x.split("_")[1][0])))
...
# %% PLOT CNR - dis, col in different plots
# reflectanceSet = {}
# for musType in sessionIDSet:
...
for musType in sessionIDSet:
    ...
    for idx, sessionID in enumerate(musType):
        ...
        for resultPath in resultPathSet:
            ...
            lns += ax[0].plot(sdsSet, reflectance.values(), color=colorset[idx], 
                              label=f"{sessionID.split('_')[1]}")
            # lns += axtmp.plot(slope_x, slope, linestyle=":", color=colorset[idx], 
            #                   label=f"{sessionID.split('_')[1]} - slope")
            # ax[0].plot(slope_x, slope, marker=".", linestyle="--", color=colorset[idx], 
