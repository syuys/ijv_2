...
projectID = "20221218_contrast_investigate_ijvdepth_sdsrange_3to40"
...
for sessionPath in sessionPathSet:
    ...
    isIncBy1 = np.all(np.diff(orderSet) == 1)
    isMaxMatch = max(orderSet) == len(jdatSet)-1
    isMinMatch = min(orderSet) == 0
    
    if isIncBy1 & isMaxMatch & isMinMatch:
